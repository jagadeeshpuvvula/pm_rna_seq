...
placenta_metabo <- read_csv("~/Documents/air_polln_rna_seq/crib_data/metabolome_paired/metabolome_conc.csv") |>
  ...
  mutate(subject_id = as.numeric(sample_description)) |> 
  #rename(subject_id = sample_description) |> 
  select(subject_id, everything()) |>
  select(-c(sample_description, parent_sample_name)) |>
...
transposed_df <- placenta_metabo |>
  ...
  mutate(subject_id = sub("^x", "", subject_id)) |>
  dplyr::rename(chem_id = subject_id)
```
...
#MetWAS
```{r}
mwas_lm_loop(feature_table = transposed_df, 
                         exp_cov_data = df_analysis,
                         exposures = names(df_analysis)[13:24],
